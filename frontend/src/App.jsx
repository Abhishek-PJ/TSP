...
function App() {
  ...
  const [sortKey, setSortKey] = useState('pct_change'); // symbol | pct_change | open | ltp | volume
  ...
  const [columnVisibility, setColumnVisibility] = useState({}); // keys: symbol, pct_change, open, ltp, volume

  useEffect(() => {
    const fromLS = (() => {
      try { return JSON.parse(localStorage.getItem('tsp_ui_state') || '{}'); } catch { return {}; }
    })();
    const sp = Object.fromEntries(searchParams.entries());
    const get = (k, def) => sp[k] ?? fromLS[k] ?? def;
    setSortKey(String(get('sortKey', 'pct_change')));
    setSortDir(String(get('sortDir', 'desc')));
  ...
  // Sync initial sorting from URL/localStorage to TanStack state
  useEffect(() => {
    setSorting([{ id: sortKey, desc: sortDir === 'desc' }]);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // When our persisted sort changes, update TanStack state
  useEffect(() => {
    setSorting([{ id: sortKey, desc: sortDir === 'desc' }]);
  }, [sortKey, sortDir]);
  ...
  const columns = useMemo(() => [
    {
      ...
      accessorKey: 'symbol',
      cell: ({ row }) => (
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="button" onClick={() => selectSymbol(row.original.symbol)} className="text-blue-600 hover:text-blue-800 font-semibold bg-transparent border-0 p-0 cursor-pointer">
          {row.original.symbol}
        </motion.button>
      ),
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Badge text={`${row.original.pct_change >= 0 ? '+' : ''}${row.original.pct_change.toFixed(2)}%`} variant={row.original.pct_change >= 0 ? 'positive' : 'negative'} />
      ),
  ...
  // Helper to render a data table for a given dataset with consistent columns/sorting
      ...
      <table className="w-full border-separate border-spacing-0 text-left">
                ...
                return (
                  <th key={header.id} className="px-4 py-3">
                    {header.isPlaceholder ? null : (
                      <button type="button" className="font-semibold hover:underline inline-flex items-center gap-1" onClick={header.column.getToggleSortingHandler()}>
        ...
        <tbody className="text-sm">
          ...
          {table.getRowModel().rows.length === 0 && (
            <tr><td colSpan={columns.length} className="px-4 py-3 text-slate-500">No data.</td></tr>
          )}
        </tbody>
      </table>
    );
      ...
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-slate-200 shadow-sm">
        <div className="w-full px-6 lg:px-10 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={fetchPicks} disabled={loading} className="inline-flex items-center gap-2 rounded-xl px-4 py-2 font-bold text-sm text-white bg-blue-600 hover:bg-blue-700 shadow-md disabled:opacity-60">
               ...
               <div className="p-4">
                 ...
                 <div className="grid gap-3">
                   {newsArticles.slice(0, 6).map((a, idx) => (
                     <motion.a whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} key={idx} href={a.url} target="_blank" rel="noreferrer" className="bg-slate-50 hover:bg-slate-100 rounded-xl border border-slate-200 p-3 text-slate-800 transition-colors">
